...
void Test::test4(){
    ...
    testList.addBack(1);
    if(testList.size() == 1){
        std::cout << "TEST 4: size returns correct value after 1 addBack: PASSED" << std::endl;
    } else{
        std::cout << "TEST 4: size returns correct value after 1 addBack: FAILED" << std::endl;
    }
}
...
void Test::test5(){
    ...
    if(isInList && (std::find(myVector.begin(), myVector.end(), 1) != myVector.end())){         //Tests if the value exists in the vector and list.
        std::cout << "TEST 5: search returns the correct boolean when the value exists: PASSED" << std::endl;
    } else{
        std::cout << "TEST 5: search returns the correct boolean when the value exists: FAILED" << std::endl;
    }
}
...
void Test::test6(){
    ...
    if(testList.size() == myVector.size()){             //Tests if the size of the vector is equal to the size of the list.
        std::cout << "TEST 6: size will return the correct value after removeback(): PASSED" << std::endl;
    } else{
        std::cout << "TEST 6: size will return the correct value after removeback(): FAILED" << std::endl;
    }
}
